...
To prevent DHCP spoofing, a `DROP` rule is added as a mandate whenever the security group is applied. As such, trying to set a DHCP server by using a secondary portable subnet doesn't work with a security group applied.
...
If your account is enabled for custom private addressing (CPA) and security groups, be aware that security groups are scoped to the account level but not the network level. Security groups are defined by IP addresses, not private networks.
