...
* If your virtual server needs access to internal services, such as an update server, network attached storage (NAS), or advanced monitoring, make sure that the security group rules accommodate traffic for those internal services. For more information, see [IBM Cloud IP ranges](/docs/infrastructure-hub?topic=infrastructure-hub-ibm-cloud-ip-ranges).
